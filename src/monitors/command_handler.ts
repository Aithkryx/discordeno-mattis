...
export const command_handler = async (message: Message) => {
  ...
  const inhibitor_results = await Promise.all(
    [...bot_cache.inhibitors.values()].map(inhibitor =>
      inhibitor(message, command, guild)
    )
  );

  if (inhibitor_results.includes(true))
    return log_command(message, guild?.name() || "DM", "Inhibibted");
...
export const check_prefix = (
  ...
  const prefix = guild_id
    ? bot_cache.guild_prefixes.get(guild_id)
    : configs.prefix;
